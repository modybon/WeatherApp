...
class ViewModel(application: Application): AndroidViewModel(application) {
 ...
 private var citiesRep = CitiesRepositoriy()
 init {
  var citiesRep = CitiesRepositoriy()
  //this.citiesWheatherList = citiesRep.allCities
  //this.citiesWheatherList = MutableLiveData<List<CityWheatherInfo>>()
  this.citiesWheatherList = MutableLiveData<ArrayList<CityWheatherInfo>>()
  cityWheatherInfo = CityWheatherInfo()
 }
 fun loadWeather(city: String){
  var ins : InputStream? = null;
  var u : String = "https://api.openweathermap.org/data/2.5/weather?q=$city&appid=${this.key}&units=$metrics"
  var result : String
  try{
   var url = URL(u)
...
//   var country = tempInfo5.get("country").toString()
 ...
 fun setUp(context: Context) =viewModelScope.launch(Dispatchers.IO) {
  ...
  SharedPrefrencesManager.readCities("Cities").forEach{
     city ->
